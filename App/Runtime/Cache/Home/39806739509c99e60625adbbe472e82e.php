...
mobile_device_detect("http://www.DTCMS.cn/index.php/mobile");

</SCRIPT>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<link rel="shortcut icon" type="image/ico" href="/favicon.ico">
<title><?php echo ($title); ?> - Powered by DTCMS</title>
<meta name="keywords" content="<?php echo ($keywords); ?>" />
<meta name="description" content="<?php echo ($description); ?>" />
<script type="text/javascript">
	var Public = "/comz/Public/Home/Default";
</script>
<script  type="text/javascript" src="/comz/Public/Home/Default/js/jquery_min.js"></script>
<script  type="text/javascript" src="/comz/Public/Home/Default/js/index.js"></script>
<script  type="text/javascript" src="/comz/Public/Home/Default/js/topdb.js"></script>
<!--[if IE 6]><script src="/comz/Public/Home/Default/js/png.js"></script><![endif]-->
<link rel="stylesheet" type="text/css" href="/comz/Public/Home/Default/css/common.css" media="all">
<link rel="stylesheet" type="text/css" href="/comz/Public/Home/Default/css/index.css" media="all">
</head>
<body>
...
<!-- 页头 -->
﻿<!--[if IE 6]>
<script src="/comz/Public/Home/Default/js/DD_belatedPNG_0.0.8a-min.js" language="javascript" type="text/javascript"></script>
<script>
...
<div class="header" >
	...
	<div class="column" style="clear:both">
    	<div class="logo"><a href="/comz/index.php"><img alt="TuziCMS-企业网站内容管理系统" height="62" src="/comz/Public/Home/Default/images/logo.png" /></a></div>			
        <div class="nav">
            <ul id="menu">
						
<?php
 $_nav_m=D('Column')->order("column_sort")->field('id,f_id,column_name,column_ename,column_url,column_type,column_sort,column_status,column_link')->where("column_status=0")->relation(true)->select(); $_nav_m=Common\Lib\Category::unlimitedForLayer($_nav_m); $modlu='/comz/index.php/home/index/index'; strpos($modlu, "mobile"); if (strpos($modlu, "mobile")==''){ foreach($_nav_m as $k3 => $v3){ if($v3['column_link']==1){ $_nav_m[$k3]['url'] = '/comz/index.php'.'/'.$v3['column_url']; } if($v3['column_link']==2){ $_nav_m[$k3]['url'] = '/comz/index.php'.'/'.$v3['column_ename']; } if($v3['column_link']==0){ $_nav_m[$k3]['url'] = '/comz/index.php'.'/'.$v3['url'].'/'.group.'/'.'id'.'/'.$v3['id']; } } }else { foreach($_nav_m as $k3 => $v3){ if($v3['column_link']==1){ $_nav_m[$k3]['url'] = '/comz/index.php'.'/'.'mobile'.'/'.$v3['column_url']; } if($v3['column_link']==2){ $_nav_m[$k3]['url'] = '/comz/index.php'.'/'.'mobile'.'/'.$v3['column_ename']; } if($v3['column_link']==0){ $_nav_m[$k3]['url'] = '/comz/index.php'.'/'.'mobile'.'/'.$v3['url'].'/'.group.'/'.'id'.'/'.$v3['id']; } } } foreach($_nav_m as $autoindex => $_nav_v): extract($_nav_v); ?><li id="dr_nav_<?php echo ($id); ?>">
                    ...
                    <div class="location">
                        <div id="wrap" class="wrap">
                            <div class="slide" id="slide">
                                <ul class="slide_ul">
									<?php
 $id=2; $topnav=M('Adnav')->where("id=$id")->select(); foreach ($topnav as $k => $v){ $where['advert_nav'] = $id; $topnav[$k]['news']=D('Advert')->where($where)->where('advert_show=0')->limit("0,5")->order("advert_sort ")->relation(true)->select(); $_result_v=$topnav[$k]['news']; } foreach($_result_v as $k2 => $v2){ $_result_v[$k2]['advert_name'] = Common\Lib\Common::substr_ext($v2['advert_name'], 0, 16, 'utf-8',""); } foreach($_result_v as $k2 => $v2){ $_result_v[$k2]['advert_image'] = '/comz'.'/'.'Uploads'.$v2['advert_image']; } foreach($_result_v as $_result_m): extract($_result_m); ?><li>									  <img src="<?php echo ($advert_image); ?>" alt="<?php echo ($advert_name); ?>" title="<?php echo ($advert_name); ?>" width="1920px" height="400px" >									</li><?php endforeach;?>
                                </ul>
                            </div>
                        </div>
                    </div>
        ...
        <script type="text/javascript" src="/comz/Public/Home/Default/js/jquery_superslide.js"></script>
        <script>
            $(function(){
                $(".j-index-slide").slide({
...
<div class="column mar-t-30  ov">
 ...
 $id=2; $topcate=M('Column')->where("id=$id")->order('column_sort')->select(); $m=M('Column')->order('column_sort')->select(); foreach ($topcate as $k => $v4){ $cids=Common\Lib\Category::getChildsId($m, $v4['id']); $cids[]=$v4['id']; } $id=1; $m=D('Attr'); $data['g.id']= $id; $where=array('nv_id'=>array('IN', $cids)); $field='g.id,g.attr_name,g.attr_color,i.news_id,i.attr_id,r.id,r.nv_id,r.news_title,r.news_content,r.news_hits,r.news_author,r.news_addtime,r.news_updatetime,r.news_sort,r.news_pic,f.column_name,m.model_table'; $result=$m->alias('g')->join('LEFT JOIN tuzi_attr_news i ON i.attr_id = g.id')->join('LEFT JOIN tuzi_news r ON r.id = i.news_id')->join('LEFT JOIN tuzi_column f ON f.id = r.nv_id')->join('LEFT JOIN tuzi_model m ON m.id = f.column_type')->field($field)->limit("0,3")->order('r.news_sort ')->where($data)->where($where)->where("news_dell=0")->select(); foreach($result as $k2 => $v2){ $result[$k2]['news_title'] = Common\Lib\Common::substr_ext($v2['news_title'], 0, 25, 'utf-8',""); } foreach($result as $k2 => $v2){ $result[$k2]['news_content'] = Common\Lib\Common::substr_ext($v2['news_content'], 0, 80, 'utf-8',""); } foreach($result as $k2 => $v2){ $pic=$v2['news_pic']; strpos($pic, "nopic"); if (strpos($pic, "nopic")==''){ $result[$k2]['news_pic'] = '/comz'.$v2['news_pic']; }else{ $result[$k2]['news_pic'] = '/comz'."/Data/Images/nopic.jpg"; } } $modlu='/comz/index.php/home/index/index'; strpos($modlu, "mobile"); if (strpos($modlu, "mobile")==''){ foreach($result as $k3 => $v3){ $result[$k3]['url'] = '/comz/index.php'.'/'.$v3['model_table'].'/'.detail.'/'.'id'.'/'.$v3['id']; } }else { foreach($result as $k3 => $v3){ $result[$k3]['url'] = '/comz/index.php'.'/'.'mobile'.'/'.$v3['model_table'].'/'.detail.'/'.'id'.'/'.$v3['id']; } } foreach($result as $result_v): extract($result_v); ?><dl>
		...
		<div class="cont_c">
			...
			<dl>
                <dt><img src="/comz/Public/Home/Default/images/company.jpg" width="138" height="93" /></dt>
				<dd>
...
<em>&copy; 2014-2015 <strong><a href="http://www.dt0572.net" target="_blank" style="color:#4CB32E;">Dt0572 Team</a></strong> 版权所有</em>
 &nbsp; &nbsp;
<em>本站由<strong><a href="http://www.dt0572.com" target="_blank" style="color:#4CB32E;"> <?php echo (C("setting.Software")); ?> </a></strong>强力驱动</em>
</div>
</div>
...
<div class="tuzikf">
    ...
    <div class="slide_box" style="display:none;">
    	<h2><img src="/comz/Public/Home/Default/images/slide_box.jpg" /></h2>
        <p><a title="点击这里给我发消息" href="http://wpa.qq.com/msgrd?v=3&amp;uin=<?php echo ($kefu_qq); ?>&amp;site=www.cactussoft.cn&amp;menu=yes" target="_blank"><img src="/comz/Public/Home/Default/images/qqweb.gif"></a></p>
        <p>
        	<img src="/comz/Public/Home/Default/images/qrcode.jpg" class="weixin"  width="120" height="120"/><br />
        	<b>客户服务热线</b><br />
        	<?php echo ($kefu_tel); ?>
        </p>
        <span><a href="/comz/index.php/guestbook">给我们留言</a></span>
    </div>
</div>
<script>
$(function(){
