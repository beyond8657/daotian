...
<div class="main">
    ...
    <div class="operate">
        <div class="left">
            <input type="button" onclick="window.open('/comz/index.php/manage/notice/add','main')" target="main" class="btn_blue" value="添加">
				<input class="btn_blue" type="button" onClick="return clear_del();"  value="删除" />      
        </div>
        <?php if(ACTION_NAME == "index"): ?><div class="left_pad">
            <form method="get" action="/comz/index.php/manage/notice/search">
    ...
    <form action="{:U(GROUP_NAME.'/Link/delAll')}" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td>
                    <a href="/comz/index.php/<?php echo ($module); ?>/Notice/edit/id/<?php echo ($v["id"]); ?>">编辑</a>
                    <a href="/comz/index.php/manage/notice/del/id/<?php echo ($v["id"]); ?>" onclick="return confirm('是否确定删除?')">删除</a>
				</td>
