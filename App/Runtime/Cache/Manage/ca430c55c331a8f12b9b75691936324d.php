...
<script src="/comz/App/Manage/View/Default/js/common.js" type="text/javascript"></script>
 <script type="text/javascript">
  function clear_del()
  {
   if(confirm("确定要删除数据吗？"))
   {
	subform('/comz/index.php/manage/user/delall');
   }
  }
 </script>
...
<div class="main">
    ...
    <div class="operate">
        <div class="left">
            <input type="button" onclick="window.open('/comz/index.php/manage/user/add','main')" target="main" class="btn_blue" value="添加">
				<input class="btn_blue" type="button" onClick="return clear_del();" value="删除" />      
        </div>
        <?php if(ACTION_NAME == "index"): ?><div class="left_pad">
            <form method="get" action="/comz/index.php/manage/user/search">
    ...
    <form action="<?php echo U(GROUP_NAME.'/Member/delAll');?>" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<?php if(is_array($vlist)): $i = 0; $__LIST__ = $vlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td>
                    <a href="/comz/index.php/manage/user/detail/id/<?php echo ($v["id"]); ?>">详情</a>
                    <a href="/comz/index.php/manage/user/edit/id/<?php echo ($v["id"]); ?>">修改密码</a>
                    <a href="/comz/index.php/manage/user/del/id/<?php echo ($v["id"]); ?>" onclick="return confirm('是否确定删除?')">删除</a>
				</td>
